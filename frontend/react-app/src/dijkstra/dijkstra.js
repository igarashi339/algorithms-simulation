...
export const dijkstraParser = (response, setControl, setResult) => {
  ...
  const colordedEdges = graph.edges.map(edge => {
  ...
  const coloredNodes = pipe(
    update(response.data.search_info.start_node, assoc('color', 'red', graph.nodes[response.data.search_info.start_node])),
    update(response.data.search_info.goal_node, assoc('color', 'red', graph.nodes[response.data.search_info.goal_node]))
  )(graph.nodes.map(node => response.data.search_info.shortest_path.includes(node.id) ? assoc('color', 'salmon', node) : node));
